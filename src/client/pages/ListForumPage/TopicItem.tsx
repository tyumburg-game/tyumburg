...
export function TopicItem(props: {topic: Nullable<TopicForum>}) {
  ...
  let d = new Date(topic.user.createdAt);
  let ye = new Intl.DateTimeFormat('en', { year: 'numeric' }).format(d);
  let mo = new Intl.DateTimeFormat('en', { month: 'short' }).format(d);
  let da = new Intl.DateTimeFormat('en', { day: '2-digit' }).format(d);
  let hours = new Intl.DateTimeFormat('ru', { hour: 'numeric' }).format(d);
  let minuts = new Intl.DateTimeFormat('ru', { minute: 'numeric' }).format(d);

  const createdAt = `${da}-${mo}-${ye} ${hours}:${minuts}`
